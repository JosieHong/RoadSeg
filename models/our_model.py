...
class Road_Seg(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.n_channels = n_channels # we don't use that but for unified format
        ...
        self.conv_transpose = nn.Sequential(
            ...
            nn.ConvTranspose2d(64, n_classes, stride=(4,4), kernel_size=16, padding=(6,6)), 
            nn.BatchNorm2d(n_classes), 
            nn.ReLU(inplace=True)
        )
        # self.fusion4 = Fusion_Module(256, n_classes, t_stride=(1,1), t_kernel=3, t_padding=(1,1))
    ...
    def forward(self, x):
        ...
        # 
        # output features' size:
        #   torch.Size([batch_size, n_classes, 256, 256])
        # -------------------------------------
        output = self.conv_transpose(output)
        # ouput = self.up(output)
